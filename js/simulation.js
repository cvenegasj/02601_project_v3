...
// Simulations modules. A set of objects defining initial presets for the simulations. 
const FOOD_FINDERS = {
  env: FOOD_HAZARD,
  numPopulations: 1,
  traits: [SCAVENGER]
...
function setupFromUI() {
  ...
  if (config.evolutionActivated) {
    evolutionInterval = setInterval(setTimeToEvolve, config.generationLifespan * 1000);
  } else {
...
function setTimeToEvolve() {
  timeToEvolve = true;
  //console.log("holi timer " + timeToEvolve);
}
